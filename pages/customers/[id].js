...
export default function EditCustomer() {
  ...
  const fetchCustomer = async () => {
    try {
      ...
      show.error('Failed to load customer')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      show.success('Customer updated successfully')
      router.push('/customers')
    } catch (error) {
      console.error('Error updating customer:', error)
      show.error('Failed to update customer')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 py-4 sm:py-6 md:py-8">
      <div className="w-full max-w-2xl mx-auto px-3 sm:px-4 md:px-6">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold">Edit Customer</h1>
          <p className="text-gray-600 text-sm sm:text-base mt-1">Update customer information</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                    <p className="font-semibold text-sm">₹{(formData.balance || 0).toFixed(2)}</p>
                  </div>
                </div>
              </div>
